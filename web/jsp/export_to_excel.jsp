<%@ page import="com.thomsonreuters.listsales.pool.ConnectionPool,java.io.*,org.apache.poi.hssf.usermodel.*,org.w3c.dom.*,javax.servlet.*,javax.xml.transform.*,javax.xml.transform.dom.*,javax.xml.transform.stream.*,javax.servlet.ServletOutputStream,java.sql.*,java.util.*,org.apache.poi.hssf.util.*,java.awt.*;" %>
...
try{
  ...
  pool = (ConnectionPool) application.getAttribute("Pool");
  connection = pool.getConnection();
   ...
   style_grey.setFillForegroundColor(color_maroon);
      ...
      //Class.forName("com.ibm.db2.jcc.DB2Driver"); //load db com.thomsonreuters.listsales.servlet if not already loaded
          //  connection = DriverManager.getConnection("jdbc:db2://sandbox.isinet.com:3754/listdb","txtsrch","ts957");
      Statement stmnt = connection.createStatement();
//  String headSql ="SELECT distinct CNTCATG from LSTSALES.ORDRRSLT where ORDERID="+orderId+" and SEARCHID="+srcId+" AND CLNID="+clnId+" and CNTNAME in ('Domestic','Foreign','12 Months/Total','3 Months','6 Months','9 Months')";
    String headSql ="SELECT distinct CNTCATG from LSTSALES.ORDRRSLT where ORDERID="+orderId+" and SEARCHID="+srcId+" AND CLNID="+clnId+" and CNTTYPE in ('D','F','T','3','6','9','X','Y','W')";
      ...
      HSSFCell cell=null;
      int i=1;
    //Starting of excel by writing headers such as customer name,serach id ,customer listname
     rw  = s.createRow(i);
     short ceNum=0;
  ...
  for(int tab=0;tab<3;tab++)
  {
    ...
    rs=stmnt.executeQuery(headSql);
      ...
      if(tab==0)
    {
      sql="SELECT CNTNAME, PCNTVALUE from LSTSALES.ORDRRSLT where ORDERID="+orderId+" and SEARCHID="+srcId+" AND CLNID="+clnId+" and CNTTYPE in ('D','F','T','3','6','9','X','Y','W')";
      cell.setCellType(HSSFCell.CELL_TYPE_STRING );
      cell.setCellValue("POSTAL");
    } else if (tab == 1) {
      sql="SELECT CNTNAME, ECNTVALUE from LSTSALES.ORDRRSLT where ORDERID="+orderId+" and SEARCHID="+srcId+" AND CLNID="+clnId+" and CNTTYPE in ('D','F','T','3','6','9','X','Y','W') order by cntseq";
      cell.setCellType(HSSFCell.CELL_TYPE_STRING );
      cell.setCellValue("Emails");
    }else {
                sql="SELECT CNTNAME, case when ICNTVALUE is null then 0 else ICNTVALUE end as ICNTVALUE from LSTSALES.ORDRRSLT where ORDERID="+orderId+" and SEARCHID="+srcId+" AND CLNID="+clnId+" and CNTTYPE in ('D','F', 'U', 'T','3','6','9','X','Y','W') order by cntseq";
      cell.setCellType(HSSFCell.CELL_TYPE_STRING );
...
while(rs.next())
    ...
    while(rs.next())
  {
    ...
    if(!temp.equals(val))
    {
      ...
      cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC );
      cell.setCellValue(((double)Integer.parseInt(rs.getString(2))));
      temp=val;
    }else
    {
      kk++;
...
String headSql2 ="SELECT distinct CNTCATG from LSTSALES.ORDRRSLT where ORDERID="+orderId+" and SEARCHID="+srcId+" AND CLNID="+clnId+" and CNTTYPE not in ('D','F','U','T','3','6','9','X','Y','W') ";
...
//int rsLen=rs.getFetchSize();
  ...
  //String query= ("sql"+tab).toString();
  String sql123="SELECT CNTNAME,PCNTVALUE, ECNTVALUE, case when ICNTVALUE is null then 0 else ICNTVALUE end as ICNTVALUE, CNTCATG from LSTSALES.ORDRRSLT where ORDERID="+orderId+" and SEARCHID="+srcId+" AND CLNID="+clnId+" and CNTTYPE not in ('D','F','U','T','3','6','9','X','Y','W')";
...
//      kk=(short)Integer.parseInt(headers.get(rs.getString(4)).toString());
                kk=(short)Integer.parseInt(headers.get(rs.getString(5)).toString());
        //this for loop is to fill the non available cell values with value 0
        /*for(short c=1;c<kk;c++)
        {
          cell=rw.createCell(c);
          cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
          cell.setCellStyle(style_yellow);
          cell.setCellValue(((double)Integer.parseInt("0")));
      ...
      cell.setCellStyle(style_grey);
      cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
      cell.setCellValue(((double)Integer.parseInt(rs.getString(3))));
            posCol_inc_3=(short)(posCol_inc * 2 +kk-2);
            cell=rw.createCell(posCol_inc_3);
            cell.setCellStyle(style_grey);
            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
            cell.setCellValue(((double)Integer.parseInt(rs.getString(4))));
      temp=val;
    }else
    {
      prev_posCol=kk;
